...
# Plot #4: Multipart plot including:
#          #4.1: Global active power   as a function of time
#          #4.2: Voltage               as a function of time
#          #4.3: Energy sub metering   as a function of time
#          #4.4: Global reactive power as a function of time
...
buildPlot4 <- function() {
    ...
    par(mfrow=c(2,2))
    # Add sub-plot #4.1:
    plot(df$datetime, df$globalactivepower, xlab="", 
    ...
    # Add sub-plot #4.3:
    plot(df$datetime, df$submetering1,
         xlab="", ylab="Energy sub metering", type="n")
    lines(df$datetime, df$submetering1, col="black")
    ...
    legend(legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),
           col=c("black","red","blue"), lty="solid", box.lty="blank",
           x=1170340000, y=41.0, bg=NULL)
    # Add sub-plot #4.4:
    plot(df$datetime, df$globalreactivepower, xlab="datetime",
         ylab="Global_reactive_power", type="n")
    lines(df$datetime, df$globalreactivepower)
